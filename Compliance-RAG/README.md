...
3. Apply the database schema in the Supabase SQL editor by pasting the contents of
   `../supabase/schema.sql` (creates the `documents` / `chunks` tables, the HNSW
   index, and the `match_chunks` function).

4. Add regulation files to `backend/data/corpus/` (e.g. `gdpr.txt`, `gdpr.pdf`).
...
Run the server from `backend/` with the venv active:
